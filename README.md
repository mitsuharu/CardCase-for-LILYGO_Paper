...
[LILYGO T5 4.7 inch e-Paper（ESP32-S3）](https://lilygo.cc/en-us/products/t5-4-7-inch-e-paper-v2-3) で、SD カードに保存した画像を選んで全画面に表示します。スマホから WiFi で送った画像・文字・名刺も同じように表示できます。名刺やメモを入れておいて、場面に合わせて出し分ける、という使い方を想定しています。
...
SD カードを抜き差ししなくても、スマホから直接送れます。専用アプリは要りません。画像のほかに、その場で書いた文字や、項目を並べた名刺も送れます。
...
- **枠が横長のときは、画像を左に置いて文字と QR を右に積みます。**このボードの画面は横長（960x540）なので、既定ではこの並びになります。`縦横を入れ替え` で縦長にすると、上から順に 1 列で積みます
