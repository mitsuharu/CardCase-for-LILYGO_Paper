...
5. 60 秒ほど操作がないとスリープに入ります。そのあとは **BOOT ボタン**で復帰します

### 操作

ボタンは BOOT（GPIO21）の 1 つだけなので、短押しと長押しで役割を分けています。

| | タッチのある H716 | タッチの無い H578 / H580 |
| --- | --- | --- |
| 一覧を動かす | 行を直接タップ / BOOT を短押し | BOOT を短押し |
| 決定 | 行をタップ / BOOT を長押し | BOOT を長押し |
| ページ送り | フッタの `< Prev` `Next >` | 短押しで送っていくと次のページへ移ります |
| 画像から戻る | 画面をタップ / BOOT を押す | BOOT を押す |
