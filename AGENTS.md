...
1. **タッチの座標変換**（`Input`）。`setSwapXY(true)` / `setMirrorXY(false, true)` は
   LilyGo の例に合わせてあるが、基板の版で変わることがある。ずれていたら `Input.cpp` の
   その 2 行だけ直す
2. **microSD がマウントできるか**（`Storage`）。SPI の 4 本は `utilities.h` の値をそのまま使っている
3. **画像の階調**（`ImageDraw`）。4×4 の組織的ディザを入れてあるが、強すぎ / 弱すぎは実機で見ないと分からない
