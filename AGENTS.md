...
lib/
  ...
  WebTransfer/   WiFi（SoftAP）で画像を受け取る。文字と名刺はブラウザ側で画像にして同じ経路で送る
...
アップロード画面には「画像」「テキスト」「名刺」がある。テキストと名刺は canvas に描いて
PNG にし、画像と同じ `/upload` へ送る。**本体側に文字を送るための経路は無い**ので、
受信・保存・表示のコードは画像のときと 1 行も変わらない。
...
からでもそのまま送れる。案内にもそう書いてある。名刺も、画像を載せなければ同じく送れる。

### 名刺は項目を並べて作る

「テキスト」が書いた文字をそのまま画像にするのに対し、「名刺」は決まった項目
（画像・タイトル・サブタイトル・SNS アカウント・QR にする URL）を受け取って並べる。

- **組み方は決め打ちにして、寄せや大きさは選ばせない。**名刺としての収まりは並べ方で
  決まっていて、触れるようにしても読みにくい組み方が増えるだけになる
- **空の項目は場所を取らない。**残った項目を詰めて中央に置く。全部埋める前提にすると、
  名前だけ載せたいときに余白が間延びする
- **枠が横長で画像があるときは、画像を左に置いて文字と QR を右に積む。**1 列に積むと
  画像が潰れ、左右が空いたままになる。縦長のときは上から順に 1 列で積む。
  **このボードの画面は横長（960x540）なので、既定では 2 列の並びになる**
- **高さの取り分は重みで割り、文字は残りをもらう**（画像 5・文字 3・QR 4）。実際に使う
  高さは中身で決まるので、重みは「どれを大きく見せるか」の目安でしかない。文字も取り分で
  切ると、行数の多い名前で「入らない」になってしまう。他が使わなかったぶんが空いている
  のに、である
- **タイトルは折り返さずに入る大きさを先に採る。**テキストの側と同じ理由で、折り返しを
  先に許すと「山田」「太郎」と名前を割ってでも字を大きくしてしまう。読めない大きさ
  （`READABLE`）まで落ちるときだけ折り返しに任せる
- サブタイトルとアカウントの大きさは、タイトルに対する比（0.55 と 0.45）で決める。
  3 つが連動するので、二分探索するのはタイトルの大きさだけでよい
- **文字の大きさは、テキストと同じく自動を基準にした割合で指定する**（40〜100%）。
  3 つが連動するので、つまみは 1 つでよい。下限が `READABLE` なのも同じ

**QR はブラウザ側で作る。**本体に QR を作る手段はあるが（接続画面で使っている `qrcode`）、
それは本体が描くためのもので、送る前のプレビューには使えない。この画面はインターネットにも
出られないので外から持ってくることもできない。用途を URL 1 本に絞り、
**8 ビットモード・誤り訂正 M・型番 1〜10**（213 バイトまで）だけを扱う。
そのぶん持つ表が短く済む。

- **長すぎる URL は描く前に弾く。**`qrFits` で先に見て送信を止める。黙って QR の無い
  名刺を作らない
- **升目は整数の大きさで描く。**電子ペーパーは階調が粗く、半端な大きさで描くと升目の
  境が濁って読めない
- **周りの余白（クワイエットゾーン 4 升）も自分で持つ。**これが無いと、読み取り機が
  符号の端を見つけられない
- **マスクは 8 通り試して、規格どおりの点数がいちばん低いものを採る。**どれを選んでも
  復号はできるが、偏った並びは読み取りにくい
- **詰め物は必ず `0xec` から始めて交互に置く。**ここがずれると誤り訂正語まで変わり、
  出来上がった QR は読めなくなる。並べ方の間違いと違って絵は正しく見えるので、
  気づきにくい
...
- WiFi の画面から文字を送って表示するところまで動く（2026-09-07）。長い 1 行を
  折り返さずに小さいまま出す場面がこのパネル（960 幅）では増えるが、実機で読めた。
  折り返しの境目（`READABLE` の 12px）はこのままでよい
- 名刺も送って表示するところまで動く（2026-09-07）。**パネルに出した QR はスマホで
  読める**（既定の 960x540・画像なしで 1 升 8px ほど）。画像を左に置く 2 列の並びも、
  名刺として収まって見える
